...
const props = defineProps({ disc:{type:Object}});
const form = useForm({
    name:props.disc.name,
    artist:props.disc.artist,
    album:props.disc.album,
    year:props.disc.year,
    gender:props.disc.gender
});
</script>

<template>
    <Head title="Edit Disc" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Disc</h2>
        </template>

        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-4 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <form @submit.prevent="form.patch(route('discs.update',disc))"
                    ...
                    <div class="mt-3">
                            ...
                            <TextInput id="artist" type="text" class="mt-1 block w-full" v-model="form.artist"
                                autocomplete="artist" />
                            <InputError class="mt-2" :message="form.errors.artist" />
                        </div>
